...
namespace RestaurantOtomasyonuLive
{
    public partial class StartScreen : Form
    {
        ...
        private void btn_login_Click(object sender, EventArgs e)
        {
            ...
            // Giriş işlemi
            if (sqlMethods.login(txt_mail.Text, txt_password.Text))  // login metodu burada başarılıysa
            {
                ...
                else if (role.Equals("personel"))
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand("SELECT e.e_position FROM person p JOIN employee e ON p.person_id = e.p_id_FK WHERE p.mail = @mail", new Connection().Connect))
                    {
                ...
                AppSession.CartId = sqlMethods.GetOrCreateCart(KullaniciBilgileri.KullaniciData.Mail);
            }
